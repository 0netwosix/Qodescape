...
class Graph:
    ...
    def create_node_label(self, existing_labels, new_label, node_name):
        with self.driver.session() as session:
            ...
            for row in result:
                Print.success_print("[200]", "[DB-Node-Label] CREATED: {label} ({labels})".format(label=new_label, labels=", ".join(row["labels"])))
    ...
    def create_node(self, node_name, node_type):
        ...
        with self.driver.session() as session:
            ...
            for row in result:
                Print.success_print("[200]", "[DB-Node] CREATED: {node}".format(node=row["p1"]))
    ...
    def find_node(self, node_name, node_type):
        with self.driver.session() as session:
            ...
            if result:
                for row in result:
                    Print.success_print("[200]", "[DB-Node] FOUND: {row}".format(row=row))
                return True
            else:
                Print.info_print("[404]", "[DB-Node] N/A: {node_name}".format(node_name=node_name))
                return False
    ...
    def find_relationship(self, parent_node, parent_node_type, child_node, child_node_type, relationship_type):
        with self.driver.session() as session:
            ...
            if result:
                for row in result:
                    Print.success_print("[200]", "[DB-Relationship] FOUND: {parent_node} {relationship_type} {child_node}".format(
                ...
                Print.info_print("[404]", "[DB-Relationship] N/A: {parent_node} {relationship_type} {child_node}".format(
                        parent_node=parent_node,
